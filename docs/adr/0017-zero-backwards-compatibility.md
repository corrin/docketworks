...
- This codebase has no callers it does not control — backend and frontend are one repository with one history, one CI and one deploy (never a submodule or a second repo), and there is no third-party API. Shared configuration lives at the root — one `.gitignore`, one `ci.yml`, one Dependabot config, one `scripts/server/deploy.sh` — and the frontend derives its origin from the backend `.env`'s `APP_DOMAIN` at build time (`frontend/vite.config.ts`), so no second configuration exists to drift. The exceptions are the externally held URLs listed in CLAUDE.md's porting rules (Xero OAuth/webhook, CRM phone ingestion, ServiceApiKey consumers); changes there are coordinated with the external party, not shimmed in code.
