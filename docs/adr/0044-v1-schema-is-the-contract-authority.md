...
For every operation both schemas expose, v2's declaration may not be weaker than v1's — v1's `frontend/schema.yml` is the authority because DRF derived it from the models, and `scripts/checks/schema_parity_diff.py` fails the build when v2 drifts weaker.
...
When you add or port an operation, run `uv run python -m scripts.checks.schema_parity_diff`. It fails on any property where v2 is weaker than v1 in one of three ways: a lost `format: uuid`, a value v1 guarantees that v2 admits null for, or a property v1 requires that v2 makes optional. The known set lives in `scripts/schema-contract-gaps.txt` and the live set must **equal** it. A new gap fails, and a recorded gap that no longer exists also fails — otherwise the file rots into a wishlist that only ever grows.
...
`scripts/checks/export_openapi.py` and `scripts/checks/schema_parity_diff.py` must read the same Django settings module (`config.settings_test`). Different modules could describe different API surfaces, leaving the parity diff guarding a schema the frontend never generates from.
