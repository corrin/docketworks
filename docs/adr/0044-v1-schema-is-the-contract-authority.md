...
When you add or port an operation, run `uv run python scripts/schema_parity_diff.py`. It fails on any property where v2 is weaker than v1 in one of three ways: a lost `format: uuid`, a value v1 guarantees that v2 admits null for, or a property v1 requires that v2 makes optional. The known set lives in `scripts/schema-contract-gaps.txt` and the live set must **equal** it. A new gap fails, and a recorded gap that no longer exists also fails — otherwise the file rots into a wishlist that only ever grows.
