...
ADR 0025 decides which regression a test guards; this one decides **whether the data can produce
it at all** — an assertion that is correct and falsifiable still proves nothing about a screen it
never drove past one page.
...
- **A volume-sensitive property is asserted in a way that holds at any row count.** Bounded and
  ...
  the client honours. State the mechanism, not the symptom.
...
- **Assert "there is a second page"** — it tests the environment instead of the code, and fails on
  a machine whose corpus is thin for reasons of its own: a retention window, a scrub, a fresh
  instance.
