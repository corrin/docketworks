...
- A handler is one arm — persist, then re-raise:
  ...
  Idempotency lives inside `persist_app_error` (it records the row on the exception instance and returns it on later calls), so a failure travelling through many layers cannot double-persist.
...
- The outermost HTTP handler picks the response status by `isinstance` on the exception's real type (most specific first) and includes the persisted id in the body via `app_error_for(exc)` (ADR 0013).
