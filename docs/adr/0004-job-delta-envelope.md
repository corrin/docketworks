...
Every Job update carries `{change_id, fields, before, after, before_checksum, etag}`; the backend verifies the checksum against current state and records the delta.
...
- The backend recomputes `before_checksum` with `compute_job_delta_checksum` over a canonical serialisation of the named fields — sorted keys, `__NULL__` sentinel for `None`, trimmed strings, normalised decimals, ISO-8601-UTC dates at millisecond precision — and rejects the request on mismatch.
- The canonicalisation exists twice, Python and TypeScript, and must stay bit-identical; golden vectors guard it. Any drift silently rejects every affected edit.
