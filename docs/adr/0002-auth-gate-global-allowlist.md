...
- Identity is resolved before the gate by Django's session `AuthenticationMiddleware`, in every environment; on `/api/` routes the ninja auth class `CookieJWTAuth` (`apps/core/auth.py`) is the authoritative check, run at operation dispatch.
