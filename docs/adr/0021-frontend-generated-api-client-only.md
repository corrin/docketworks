...
- After a backend schema change, export the wire contract with `uv run python -m scripts.checks.export_openapi` and then regenerate the client with `npm run gen:api`. Both run as commit hooks, so a stale client fails the commit tier rather than reaching CI, and the `frontend-boundary` hook (`frontend/scripts/check-api-boundary.mjs`) fails a raw HTTP call made outside `src/api`.
