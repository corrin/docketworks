...
- If a value involves the database, business rules, or an external system, the backend computes it and the frontend renders it as received — never recomputes it. (One protocol-parity exception: the delta-checksum canonicalisation of ADR 0004 exists in TypeScript too — it is protocol plumbing, not a business value.) That is what makes every total read the same in the screen, the export, and the database; a "just sum it locally" shortcut disagrees with the canonical figure the day someone runs a report.
