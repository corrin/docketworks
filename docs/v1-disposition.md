...
| `pull_prod_files.sh` | ported | `scripts/ops/pull_prod_files.sh`. The blocker was the replay storage decision: payloads live on disk under `SESSION_REPLAY_STORAGE_ROOT`, so all three destinations now exist and the script has somewhere to put them. v2 takes host and instance-user as required arguments rather than defaulting to MSM production, and validates instance-user as a plain unix account name — it is interpolated into the remote `--rsync-path`, the same escalation `pull_prod_backup.sh` guards. Still not needed by the E2E suite (the job-attachments spec uploads its own fixture); `scripts/ops/recreate_jobfiles.py` covers job-file rows restored without their bytes. |
...
| `setup_demo_payroll.py` | dropped | Retrofitted tax code, KiwiSaver, bank account and leave onto payroll employees something else had already created. `apps/xero/payroll_employees.py::create_payroll_employee` now sets all four as part of creating the employee, because Xero will not put an employee lacking any of them into a pay run — so there is nothing left to retrofit. v1 carried the same writes twice (here and in `payroll.py`) and its sync called both; v2 has one (ADR 0039). The fake IRD and bank generators moved to `apps/timesheet/services/demo_payroll_data.py`, backed by `python-stdnum`. |
...
| `purge_old_session_replays_daily` | ported | `config/celery.py`, 01:30 NZT daily, running `apps.diagnostics.tasks.purge_old_session_replays_task`. Held back while nothing could populate a replay; scheduled with ingestion, and now deletes chunk payloads and the recording directory as well as the rows, since retention is the feature's only privacy control. |
...
v1's `xero` command carried fifteen flags; setup is ported, employee import is
part of the generic sync, and demo-only employee creation belongs to
`seed_xero_from_database --only employees`. The remaining flags are either
dropped because those workflows now have one canonical implementation or are
blocked by a named feature such as Xero Projects. They are recorded here rather
than in the command, which names only its own supported actions.
...
| `--payroll-employees`, `--payroll-rates`, `--payroll-calendars`, `--payroll-leave-types`, `--payroll-pay-runs` | dropped | Read-only listings of the payroll objects, one flag each, for checking what an organisation actually holds when a payroll operation refuses. Every listed object is now readable through the application's own surfaces — employees on the staff list, earnings rates and leave types on Leave settings and `xero --configure-payroll`, the calendar validated by `xero --setup`, pay runs on the weekly timesheet panel and its mirror — and `scripts/ops/xero_payroll_probe.py` remains the raw diagnostic for what the API itself answers. A second operator-facing print of the same objects would be a parallel implementation (ADR 0039). |
...
| `--import-staff` (+ `-dry-run`, `-password`) | ported differently | Payroll employees are now an ordinary inbound Xero sync entity. New Staff rows receive the payroll email as both office and payroll email and an unusable password; an administrator explicitly provisions access. There is no separate importer, password flag or working-pattern import. |
| `--force` | dropped | The generic sync already has its explicit setup-only `force` gate; there is no separate staff-import safety bypass. |
