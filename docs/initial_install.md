...
Wires up the pre-commit, post-merge, and post-rewrite git hooks (the latter two regenerate the autogenerated codesight context maps after `git pull` / `git pull --rebase`) and seeds the maps for the first time:
