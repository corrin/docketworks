...
4. **Validation-failure path**: POST a client that fails `validate_for_xero()` (e.g. missing required field). Expect **HTTP 500** (not 400 — `ClientRestService.create_client` wraps non-`AlreadyLoggedException`s via `persist_and_raise`, which raises `AlreadyLoggedException`; the view's `except ValueError` doesn't match, so it falls to `except Exception` → `_build_server_error_response` → 500 with a JSON error body + `error_id`). Confirm: at least one `AppError` row persisted (the provider's `persist_app_error`); no local `Client` row orphaned with `xero_contact_id=NULL`.
5. **Xero-side failure path**: simulate a Xero API exception (e.g. temporarily point at an invalid tenant, or mock at the test layer). Same shape as step 4 — propagates to `provider.create_contact`, persisted, surfaces as `ContactResult(success=False, …)`, service raises `ValueError`, outer wrapper converts to `AlreadyLoggedException`, view returns **HTTP 500** with structured JSON.
