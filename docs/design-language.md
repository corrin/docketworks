...
| Local action styling bypasses shared buttons | [CRM people](../frontend/src/features/crm/PeopleDirectoryPage.tsx), [company details](../frontend/src/features/crm/CompanyDetailPage.tsx) and [navbar](../frontend/src/features/shell/AppNavbar.tsx) hand-style actions. Call the shared button, including for button-styled links |
...
| Embedded quoting conversation | GPT: ChatKit owns message layout, composer and history controls. The job tab keeps its 24px gutters and available width; typography is configured through the SDK. Its internal conversation scrolling and readable message width are appropriate for chat, and do not authorize width caps on entry grids |
