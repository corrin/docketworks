 ...
 *
 * Ported from v1 (scripts/report-patterns.js). Adaptation: the default spec
 * is schema.v2.yml — the exported schema openapi-ts.config.ts generates the
 * client from (v1's was schema.yml).
 */
...
async function main() {
  const specFile = process.argv[2] ?? 'schema.v2.yml'
