// Puppeteer performance capture against UAT/production (reads APP_URL from
// frontend/.env.<env>). Ported from v1 (scripts/capture_metrics.cjs)
// unchanged; analyze_captures.cjs compares the JSON captures this writes.
...
captureMetrics(env, jobId).catch((error) => {
  console.error(error)
  process.exitCode = 1
})
