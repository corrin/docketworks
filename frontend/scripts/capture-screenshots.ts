/**
 * Capture documentation screenshots for the Operations Manual.
 *
 * The capture half of the screenshot pipeline: drives Playwright through the
 * running app and writes PNGs plus a manifest. The push half is
 * scripts/gdocs/set_doc_screenshot.py, which uploads a PNG into a Google Doc
 * at its {{screenshot:<id>}} marker.
 *
 * Run with the full stack already serving (this script starts no servers):
 *   E2E_BASE_URL=http://localhost:4173 npm run manual:screenshots
 * Single ad-hoc capture:
 *   npm run manual:screenshots -- --url /kanban --output /tmp/kanban.png
 */
...
// Same env layering as playwright.config.ts: frontend/.env first, .env.test
// overriding — that is where E2E_TEST_USERNAME / E2E_TEST_PASSWORD live.
dotenv.config({ path: path.join(scriptDir, '..', '.env') })
const testEnvPath = path.join(scriptDir, '..', '.env.test')
...
const autoId = (id: string): string => `[data-automation-id="${id}"]`

// Open the first job card on the kanban board and wait for the job view.
// v2 job tabs are plain buttons stamped JobViewTabs-<key>, not role=tab.
function openFirstJob(tab?: string): (page: Page) => Promise<void> {
  return async (page: Page) => {
    const firstJob = page.locator('[data-job-id]').first()
    if (await firstJob.isVisible()) {
      await firstJob.click()
      await page.waitForSelector(autoId('JobView-job-number'), { timeout: 10000 })
      if (tab) {
        const tabButton = page.locator(autoId(`JobViewTabs-${tab}`))
...
// Screens with no v2 route (v1's people list, admin pages, KPI and payroll
// reconciliation reports) are not listed; the reports section covers the two
// reports v2 serves (/reports/wip, /reports/job-movement).
const SCREENSHOTS: ScreenshotDef[] = [
  // === LOGIN ===
  {
    ...
    route: '/login',
    waitFor: autoId('LoginView-username'),
  },
  ...
  // === JOB CREATION ===
  {
    ...
    route: '/jobs/create',
    waitFor: autoId('JobCreateView-title'),
  },
  ...
  {
    id: 'job-actual-tab',
    description: 'Job Actual tab showing real costs and revenue',
    route: '/kanban',
    waitFor: '[data-kanban-status]',
    prepare: openFirstJob('actual'),
    fullPage: true,
  },
  ...
  // === JOB ATTACHMENTS TAB ===
  {
    ...
    route: '/kanban',
    waitFor: '[data-kanban-status]',
    prepare: openFirstJob('attachments'),
  },
  ...
  },
  {
    ...
    description: 'Daily timesheet overview with staff hours',
  ...
  // === PURCHASING ===
  {
    ...
    route: '/purchasing/po',
    waitFor: autoId('PurchaseOrderView-new-po'),
  },
  {
    ...
    waitFor: autoId('PoCreateView-title'),
  },
  {
    id: 'purchasing-stock',
    description: 'Stock list with search',
    route: '/purchasing/stock',
    waitFor: autoId('StockView-search'),
  },

  // === REPORTS ===
  {
    id: 'wip-report-overview',
    description: 'Work in Progress report with summary cards and per-job table',
    route: '/reports/wip',
    waitFor: autoId('WIPReport-title'),
    fullPage: true,
  },
  {
    id: 'job-movement-report',
    description: 'Job Movement report with fortnight summary cards',
    route: '/reports/job-movement',
    waitFor: autoId('JobMovementReport-title'),
    fullPage: true,
  },
  ...
  // === CRM ===
  {
    ...
    route: '/crm/companies',
    waitFor: autoId('CompaniesTable-search'),
  },
]

// Output paths
const OUTPUT_DIR = path.resolve(scriptDir, '../manual/public/screenshots')
const MANIFEST_PATH = path.resolve(scriptDir, '../manual/screenshot-manifest.json')

function getBaseUrl(): string {
  // Required, never defaulted (ADR 0015): this script starts no servers, so a
  // baked-in localhost:4173 would silently screenshot whatever stale preview
  // happens to be listening — or error pages when nothing is.
  const baseUrl = process.env.E2E_BASE_URL
  if (!baseUrl) {
    throw new Error(
      'E2E_BASE_URL must be set to the running app, e.g. http://localhost:4173 ' +
        'when `npm run preview:e2e` is serving.',
    )
  }
  return baseUrl
}
...
function parseCliOptions(argv: string[]): CliOptions {
  ...
  for (const flag of ['url', 'output', 'wait-for'] as const) {
    if (values[flag] === '') {
...
async function authenticate(page: Page): Promise<void> {
  ...
  if (!username || !password) {
    throw new Error(
      'E2E_TEST_USERNAME and E2E_TEST_PASSWORD must be set in .env or .env.test\n' +
  ...
  await page.locator(autoId('LoginView-username')).fill(username)
  await page.locator(autoId('LoginView-password')).fill(password)
  await page.locator(autoId('LoginView-submit')).click()
