...
test.describe.serial('kanban mobile', () => {
  test('change status via drawer', async ({ authenticatedPage: page, sharedEditJobUrl }) => {
    ...
    // Selector rename (PR C decision): v2 stamps the column contract as
    // `data-kanban-status`, not v1's bare `data-status` — TanStack Router's
    // Link stamps its own `data-status="active"` on the active nav link,
    // which a bare `[data-status]` query would also match (KanbanColumn.tsx
    // header comment; rewrite-status trap 4).
    const targetCard = page
      .locator(`[data-kanban-status="${targetKey}"] [data-job-id="${jobId}"]`)
      .first()
