...
function extractUploadedId(payload: unknown): string {
  if (isRecord(payload) && Array.isArray(payload.uploaded)) {
    const first: unknown = payload.uploaded[0]
    if (isRecord(first) && typeof first.id === 'string' && first.id !== '') {
      return first.id
    }
  }
  throw new Error(`Upload response missing uploaded[0].id: ${JSON.stringify(payload)}`)
}
...
test.describe('job attachments', () => {
  ...
  test('upload and delete job attachments', async ({
    ...
    const uploadedFileId = extractUploadedId(await uploadResponse.json())
  ...
  test('uploaded attachment downloads with identical sha256', async ({
    ...
    // JobAttachmentsTab's download opens the blob in a new tab and calls
