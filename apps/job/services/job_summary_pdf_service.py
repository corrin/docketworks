...
class JobSummaryPdfService:
    """Write the canonical workshop PDF into the job's local workflow folder."""

    @staticmethod
    def refresh(job_id: UUID | str) -> None:
        """Regenerate and persist JobSummary.pdf for one job."""
    ...
    def refresh_stale(limit: int) -> tuple[int, bool]:
        """Refresh up to ``limit`` stale summaries; report (count, more remaining)."""
        if limit < 1:
            raise ValueError("Job summary PDF reconcile limit must be positive")

        job_ids = list(JobSummaryPdfService._stale_jobs().values_list("id", flat=True)[: limit + 1])
    ...
    def _stale_jobs() -> QuerySet[Job]:
        ...
        return Job.objects.filter(Q(id__in=stale_job_ids) | ~Q(id__in=summary_job_ids)).order_by(
            "-updated_at"
        )
