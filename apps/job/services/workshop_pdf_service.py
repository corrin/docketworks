...
NewPageCallback = Callable[[canvas.Canvas], float]


class StaffHoursEntry(TypedDict):
    """One staff member's summed production hours on the actual cost set."""

    name: str
    hours: float


class SubtypeBreakdownEntry(TypedDict):
    """Budget/used/remaining production hours for one labour subtype."""
...
class TimeBreakdownData(TypedDict):
    """Detailed workshop time breakdown for the workshop PDF."""

    budgeted_hours: float
    used_hours: float
    remaining_hours: float
    production_budgeted_hours: float
    production_used_hours: float
    production_remaining_hours: float
    is_over_budget: bool
    staff_breakdown: list[StaffHoursEntry]
    subtype_breakdown: list[SubtypeBreakdownEntry]


class _TimeBucket(TypedDict):
    """Accumulated hours per labour subtype (internal)."""
...
def get_job_for_workshop_pdf(job_id: UUID) -> Job:
    ...
    return (
        ...
        .annotate(**_primary_phone_annotations())
        .prefetch_related(
            # Prefetch cost lines for all three cost sets uniformly. The quote
            # is only read on the rare zero-estimate-hours fallback, so its
            # prefetch is sometimes unused — a deliberate, cheap (~5 rows)
            # eager load.
            Prefetch(
...
def _production_time_lines(cost_set: CostSet) -> list[CostLine]:
    ...
    for line in _cost_lines_for_pdf(cost_set):
        ...
        if line.labour_subtype.counts_for_scheduling:
            lines.append(line)
        # Office/admin-like labour does not consume production capacity.
    return lines
...
def format_hours_display(hours: float | None) -> str:
...
def get_workshop_hours(job: Job) -> float:
    ...
    Sums time CostLines whose labour subtype consumes schedulable staff
    capacity. Falls back to quote if estimate has zero workshop hours.
    """
    estimate = job.latest_estimate
    if estimate is None:
        raise ValueError(f"Job {job.job_number} has no estimate CostSet")
    ...
    if workshop_hours <= 0:
        ...
        if quote is None:
            raise ValueError(f"Job {job.job_number} has no quote CostSet to fall back to")
...
def _production_time_by_bucket(cost_set: CostSet) -> dict[str, _TimeBucket]:
    buckets: dict[str, _TimeBucket] = {}
    for line in _production_time_lines(cost_set):
        ...
        bucket = buckets.setdefault(
            ...
            {
                "name": (
                    "WORKSHOP TIME" if labour_subtype.is_workshop else f"{labour_subtype.name} Time"
                ),
                "hours": 0.0,
...
def _staff_breakdown_for_actual(cost_set: CostSet, job: Job) -> list[StaffHoursEntry]:
    """Group the actual production time lines by staff, hours descending."""
    staff_hours: defaultdict[str, float] = defaultdict(float)
    for line in _production_time_lines(cost_set):
        staff = line.staff
        if staff is None:
            raise ValueError(f"CostLine {line.id} for job {job.job_number} has no staff set")
    ...
    return [
        ...
        for name, hours in sorted(staff_hours.items(), key=lambda x: x[1], reverse=True)
    ]


def _subtype_breakdown(
    budgeted_by_subtype: dict[str, _TimeBucket],
    used_by_subtype: dict[str, _TimeBucket],
) -> list[SubtypeBreakdownEntry]:
    """Merge budgeted and used buckets into ordered per-subtype rows."""
    subtype_ids = list(budgeted_by_subtype)
    subtype_ids.extend(
        subtype_id for subtype_id in used_by_subtype if subtype_id not in subtype_ids
    )
    breakdown: list[SubtypeBreakdownEntry] = []
    for subtype_id in subtype_ids:
        budgeted_item = budgeted_by_subtype.get(subtype_id)
        used_item = used_by_subtype.get(subtype_id)
        fallback: _TimeBucket = {"name": "WORKSHOP TIME", "hours": 0.0, "is_workshop": False}
        name = str((budgeted_item or used_item or fallback)["name"])
        estimated = float(budgeted_item["hours"]) if budgeted_item else 0.0
        used = float(used_item["hours"]) if used_item else 0.0
        is_workshop = bool((budgeted_item or used_item or fallback)["is_workshop"])
...
def get_time_breakdown(job: Job) -> TimeBreakdownData:
    """Get detailed time breakdown for the workshop PDF (see TimeBreakdownData)."""
    ...
    if job.latest_actual:
        ...
        staff_breakdown = _staff_breakdown_for_actual(job.latest_actual, job)

    remaining_hours = budgeted_hours - used_hours
    is_over_budget = used_hours > budgeted_hours > 0
    ...
    return {
        ...
        "subtype_breakdown": _subtype_breakdown(budgeted_by_subtype, used_by_subtype),
    }
...
def _advance_to_new_page(
    pdf: canvas.Canvas,
    margin: float = MARGIN,
    on_new_page: NewPageCallback | None = None,
) -> float:
...
def draw_table_with_page_breaks(  # noqa: PLR0913 -- Layout knobs are explicit keyword-only arguments.
    ...
    *,
    ...
    Handles page breaks when the table would overflow the remaining space.
    """
    pending_parts: list[Flowable] = [table]
    ...
    while pending_parts:
        ...
        while True:
            ...
            if not parts:
                ...
                parts = current_table.split(available_width, available_height)
                if not parts:
                    # If nothing fits even on a fresh page, force-draw to
                    # avoid an infinite loop.
                    _, forced_height = current_table.wrapOn(pdf, available_width, available_height)
            ...
            # Queue remaining chunks (if any) and advance to a fresh page.
...
def wait_until_file_ready(file_path: str, max_wait: int = 10) -> None:
...
def _fit_dimensions(image_path: str, max_width: float, max_height: float) -> tuple[float, float]:
    """Return (width_pt, height_pt) to draw the image inside the bounding box.

    Preserves aspect ratio; the on-page footprint is determined by the
    bounding box, not by the source pixel count.
    """
...
_REPORTLAB_ALLOWED_TAGS = {"b", "i", "u", "strike", "link", "font", "br"}


def _convert_block_tags(soup: BeautifulSoup) -> None:
    """Convert headings, blockquotes and pre blocks to inline ReportLab markup."""
...
def _convert_lists(soup: BeautifulSoup) -> None:
    """Flatten ol/ul lists to numbered / bulleted text lines."""
    for ol in soup.find_all("ol"):
        ...
        for i, li in enumerate(items):
            ...
            replacement = NavigableString(str(replacement) + prefix + li_content + "\n")
    ...
    for ul in soup.find_all("ul"):
        ...
        for li in items:
            li_content = "".join(str(c) for c in li.contents)
            replacement = NavigableString(str(replacement) + "• " + li_content + "\n")
        ul.replace_with(replacement)


def _convert_inline_tags(soup: BeautifulSoup) -> None:
    """Map inline HTML tags onto their ReportLab equivalents and strip attrs."""
    ...
    for tag in soup.find_all(["b", "i", "u", "strike", "font", "br"]):
        if tag.name == "font":
            ...
            tag.attrs = {k: v for k, v in tag.attrs.items() if k in ("size", "face")}
        else:
            tag.attrs = {}

    # Unwrap (remove tag, keep content) any tags not in the allowed set.
    # Use list() to avoid modifying while iterating.
    for tag in list(soup.find_all(True)):
        if isinstance(tag, Tag) and tag.name not in _REPORTLAB_ALLOWED_TAGS:
            tag.unwrap()
...
def convert_html_to_reportlab(html_content: str) -> str:
    ...
    try:
        ...
        _convert_block_tags(soup)
        # Lists must be processed before unwrapping other tags
        _convert_lists(soup)
        _convert_inline_tags(soup)

        # Convert to string - BeautifulSoup renders <br> as <br/>
        result = str(soup)

        # Convert all newlines to line breaks (preserves paragraph structure
        # from <p> tags)
        ...
        logger.exception(
            "Failed to convert HTML to ReportLab markup. Input: %s",
...
def create_workshop_pdf(job: Job) -> BytesIO:
    ...
    try:
        ...
        image_files = [f for f in files_to_print if (f.mime_type or "").startswith("image/")]
        ...
        logger.exception("Error creating workshop PDF")
        persist_app_error(exc, AppErrorContext(job_id=str(job.id)))
        raise


def create_delivery_docket_pdf(job: Job) -> BytesIO:
    """Generate a delivery docket PDF.

    No materials, workshop time, or internal notes. Includes a handover
    section with signature, date, and notes fields. Does not include job
    attachments — delivery dockets are kept minimal.
    """
...
def create_workshop_main_document(job: Job) -> BytesIO:
    """Create the workshop cover: header, details, time used, materials tables."""
    buffer = BytesIO()
...
def create_delivery_docket_main_document(job: Job) -> BytesIO:
    """Create the delivery docket with two copies: Company Copy and Customer Copy."""
    buffer = BytesIO()
    pdf = canvas.Canvas(buffer, pagesize=A4)

    # Get company acronym for copy labels
    company = CompanyDefaults.get_solo()
    copy_label = f"{company.company_acronym} Copy" if company.company_acronym else "Office Copy"

    # First page - Company Copy
    y_position = PAGE_HEIGHT - MARGIN
    y_position = add_letterhead_banner(pdf, y_position)
    y_position = add_title(pdf, y_position, job, title_prefix=f"DELIVERY DOCKET - {copy_label}")
    ...
    y_position = PAGE_HEIGHT - MARGIN
    y_position = add_letterhead_banner(pdf, y_position)
    y_position = add_title(pdf, y_position, job, title_prefix="DELIVERY DOCKET - Customer Copy")
    y_position = add_delivery_docket_details_table(pdf, y_position, job)
    add_handover_section(pdf, job)
...
def add_letterhead_banner(
    ...
    # Left side: address
    address_parts = [p for p in [company.address_line1, company.address_line2, company.city] if p]
    ...
    right_parts = [p for p in [_primary_company_endpoint_number(), company.company_email] if p]
    if right_parts:
...
def add_title(
    ...
    """Render the job title (optionally prefixed) with the consistent palette."""
    ...
    if title_prefix:
        prefix_lines = _wrap_text_for_canvas(pdf, title_prefix, CONTENT_WIDTH, font_name, font_size)
        for line in prefix_lines:
            pdf.drawString(MARGIN, current_y, line)
            current_y -= line_height

    job_lines = _wrap_text_for_canvas(pdf, job_line, CONTENT_WIDTH, font_name, font_size)
...
def add_time_used_table(pdf: canvas.Canvas, y_position: float, job: Job) -> float:
    """Render the time used table: staff members, hours worked, running remaining.
    ...
    time_data.extend([["", "", ""] for _ in range(5)])
...
def _approval_age_display(job: Job) -> str:
    ...
    approved_date = timezone.localtime(approved_at, timezone.get_current_timezone()).date()
...
def _workshop_contact_table(job: Job) -> Table:
    """Build the blue contact header row (company + person/phone)."""
...
def _workshop_meta_table(job: Job) -> Table:
    """Build the due/order/pricing/approved/tradeoff meta strip."""
    due_date = job.delivery_date.strftime("%a, %d %b %Y") if job.delivery_date else "N/A"
    ...
    meta_table = Table(
        [
            [_plain_paragraph(label, brief_label_style) for label, _value in meta_items],
            [_plain_paragraph(value, brief_value_style) for _label, value in meta_items],
        ],
...
def _workshop_labour_table(job: Job) -> Table:
    """Build the labour budget table (per-subtype budget/remaining)."""
    time_breakdown = get_time_breakdown(job)
    labour_rows = [
        [
    ...
    for subtype_entry in time_breakdown["subtype_breakdown"]:
        style = brief_value_bold_style if subtype_entry["is_workshop"] else brief_value_style
        labour_rows.append(
            [
...
def add_workshop_details_table(pdf: canvas.Canvas, y_position: float, job: Job) -> float:
    """Render the workshop brief: identity, constraints, labour budget, specs."""
    y_position = _draw_table(pdf, _workshop_contact_table(job), y_position)
    y_position = _draw_table(pdf, _workshop_meta_table(job), y_position)
    y_position = _draw_table(pdf, _workshop_labour_table(job), y_position - 8)
...
def add_delivery_docket_details_table(pdf: canvas.Canvas, y_position: float, job: Job) -> float:
    ...
    job_details: list[list[Paragraph | str]] = [
        [
            _plain_paragraph(company_name, header_company_style),
            Paragraph(person_info, header_contact_style),
        ],
        ...
        [
            ...
            timezone.localtime(job.created_at, timezone.get_current_timezone()).strftime(
                "%a, %d %b %Y"
            ),
        ],
        [
...
def add_handover_section(pdf: canvas.Canvas, job: Job) -> None:
    ...
    pdf.setFillColor(TEXT_DARK)
    delivery_date = timezone.localtime(timezone.now(), timezone.get_current_timezone()).strftime(
        "%a, %d %b %Y"
    )
...
def add_materials_used_table(pdf: canvas.Canvas, y_position: float, job: Job) -> float:
    """Render the materials notes table with actual materials plus blank rows."""
    materials_data: list[list[str]] = [["DESCRIPTION", "QUANTITY", "RETAIL PRICE"]]
    ...
    # Minimum space needed to avoid orphaning the materials section:
    # "Materials Used" heading (39pt) + header row (~29pt) + 5 data rows
    # (~31pt each = 155pt) => ~223pt for heading + full table.
...
def create_image_document(image_files: list[JobFile]) -> BytesIO:
    ...
    for i, job_file in enumerate(image_files):
        file_path = str(Path(settings.DROPBOX_WORKFLOW_FOLDER) / job_file.file_path)
        if not Path(file_path).exists():
            continue

        try:
            width, height = _fit_dimensions(file_path, CONTENT_WIDTH, PAGE_HEIGHT - 2 * MARGIN - 50)
            x = MARGIN + (CONTENT_WIDTH - width) / 2
...
def get_pdf_file_paths(pdf_files: list[JobFile]) -> list[str]:
    ...
    for job_file in pdf_files:
        file_path = Path(settings.DROPBOX_WORKFLOW_FOLDER) / job_file.file_path
        if file_path.exists():
            file_paths.append(str(file_path))
    return file_paths


def merge_pdfs(pdf_sources: list[BytesIO | str]) -> BytesIO:
    ...
    try:
        ...
        for buffer in buffers_to_close:
            try:
                ...
                logger.exception("Error closing buffer")
                persist_app_error(e)
