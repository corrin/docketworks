...
class JobRestServiceClientPhoneTests(BaseTestCase):
    """Guards the client phone the job-settings tab reads from job payloads."""

    def _job_with_client_phone(self) -> Job:
    ...
    def test_get_job_for_edit_includes_client_phone(self) -> None:
        ...
        self.assertEqual(result["job"]["client_phone"], "09 222 2222")

    def test_get_job_summary_includes_client_phone(self) -> None:
        job = self._job_with_client_phone()

        result = JobRestService.get_job_summary(job.id, request=None)
