"""CostSet and CostLine: the versioned costing snapshots for a job."""
...
class CostSet(models.Model):
    ...
    #   3. QuoteImportStatusResponseSerializer
    #      in apps/job/serializers/costing_serializer.py (subset)
    ...
    COSTSET_ALL_FIELDS: ClassVar[list[str]] = [
        "id",
        "job",
        "kind",
        "rev",
    ...
    def clean(self) -> None:
        """Validate that the revision is non-negative."""
        if self.rev < 0:
            raise ValidationError("Revision must be non-negative")

    @property
    def total_cost(self) -> Decimal | int:
        """Total internal cost for all cost lines in this set (int 0 when empty)."""
...
class CostLine(models.Model):
    ...
    COSTLINE_ALL_FIELDS: ClassVar[list[str]] = COSTLINE_API_FIELDS + COSTLINE_INTERNAL_FIELDS
    ...
    approved = models.BooleanField(
        default=True,
        help_text=(
            "Indicates whether this line is approved or not by an office staff "
            "(when the line is created by a workshop worker)"
        ),
    )
    ...
    class Meta:
        ...
        constraints: ClassVar[list[models.BaseConstraint]] = [
            ...
            models.CheckConstraint(condition=~Q(desc=""), name="desc_not_blank"),
            models.CheckConstraint(
    ...
    def total_rev(self) -> Decimal:
        """Calculate total revenue (quantity * unit revenue)."""
        return self.quantity * self.unit_rev

    def clean(self) -> None:
        """Validate kind-dependent field requirements and JSON payloads."""
        super().clean()
    ...
    def _sequence_group_changed(self) -> bool:
        ...
        return (
            previous.staff_id != self.staff_id or previous.accounting_date != self.accounting_date
        )
