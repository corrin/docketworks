"""Labour subtypes and per-job charge-out rates."""
...
class LabourSubtype(models.Model):
    """A configurable category of labour.

    Categories include Workshop, Admin, Quoting, Onsite, Supervision, etc.
    Every time CostLine belongs to exactly one subtype. Subtypes carry the
    company-level default charge-out rate used to seed per-job rates
    (JobLabourRate). The is_workshop flag identifies the default workshop
    subtype; counts_for_scheduling identifies labour that consumes the
    workshop staff pool.
    """
    ...
    def default_workshop(cls) -> "LabourSubtype":
        """Return the first active workshop subtype by display order."""
        subtype = (
            cls.objects.filter(is_active=True, is_workshop=True).order_by("display_order").first()
        )
        if subtype is None:
    ...
    def default_non_workshop(cls) -> "LabourSubtype":
        """Return the default active non-workshop subtype for office/admin time."""
        subtype = cls.objects.filter(name="Admin", is_active=True, is_workshop=False).first()
        if subtype is not None:
            return subtype

        subtype = (
            cls.objects.filter(is_active=True, is_workshop=False).order_by("display_order").first()
        )
        if subtype is None:
...
class JobLabourRate(models.Model):
    """The charge-out rate for one labour subtype on one job.

    Seeded at job creation from LabourSubtype.default_charge_out_rate (zero
    for shop jobs) and editable per job afterwards. Replaces the old
    Job.charge_out_rate.
    """

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    job = models.ForeignKey("job.Job", on_delete=models.CASCADE, related_name="labour_rates")
    labour_subtype = models.ForeignKey(
        LabourSubtype, on_delete=models.PROTECT, related_name="job_rates"
    )
    charge_out_rate = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal("0.00"))
