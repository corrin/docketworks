...
``backport_data_backup`` pipeline both write it through this function — a
second sidecar convention (the retired ``.sha`` release-pointer, which no
restore path ever read) is what this module exists to prevent.
"""
