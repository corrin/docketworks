...
class TestJobPickers:
    def test_all_jobs_flags_the_stock_holding_job(
        ...
        body = api.get("/api/purchasing/all-jobs/").json()
    ...
    def test_all_jobs_excludes_archived_jobs(
        ...
        ids = {row["id"] for row in api.get("/api/purchasing/all-jobs/").json()["jobs"]}
    ...
    def test_a_search_term_reaches_an_archived_job(
        ...
        rows = api.get(f"/api/purchasing/all-jobs/?q={job.name}").json()["jobs"]
    ...
    def test_a_search_term_below_the_minimum_is_a_400(
        ...
        assert api.get("/api/purchasing/all-jobs/?q=ab").status_code == 400
    ...
    def test_purchasing_jobs_lists_costable_jobs_with_their_actual_cost_set(
        ...
        rows = api.get("/api/purchasing/jobs/").json()
    ...
    def test_purchasing_jobs_excludes_jobs_in_non_costable_states(
        ...
        assert api.get("/api/purchasing/jobs/").json() == []
...
class TestSupplierSearch:
    ...
    def test_a_name_query_finds_the_supplier(self, api: Client) -> None:
        wanted = make_company("Steel & Tube")
        make_company("Completely Different Ltd")

        body = api.get("/api/purchasing/suppliers/search/?q=steel and tube").json()

        assert [row["name"] for row in body["results"]] == [wanted.name]

    def test_an_alias_matches_too(self, api: Client) -> None:
        supplier = make_company("Pacific Steel Limited")
        SupplierSearchAlias.objects.create(company=supplier, alias="PSL")

        body = api.get("/api/purchasing/suppliers/search/?q=PSL").json()

        assert [row["name"] for row in body["results"]] == [supplier.name]

    def test_archived_and_merged_companies_are_excluded(self, api: Client) -> None:
        ...
        body = api.get("/api/purchasing/suppliers/search/?q=steel").json()
    ...
    def test_pagination_reports_totals(self, api: Client) -> None:
        for index in range(3):
            make_company(f"Paged Steel {index}")

        body = api.get("/api/purchasing/suppliers/search/?q=paged&page=2&page_size=2").json()
...
class TestPurchaseOrderPdf:
    def test_streams_a_pdf_attachment(
        self, api: Client, supplier: Company, company_defaults: CompanyDefaults, job: Job
    ) -> None:
        ...
        response = api.get(f"/api/purchasing/purchase-orders/{po.id}/pdf/")
    ...
    @pytest.mark.usefixtures("company_defaults")
    def test_a_missing_logo_is_a_transparent_500(self, api: Client) -> None:
    ...
    @pytest.mark.usefixtures("company_defaults")
    def test_an_unknown_purchase_order_is_404(self, api: Client) -> None:
        assert api.get(f"/api/purchasing/purchase-orders/{uuid4()}/pdf/").status_code == 404


@pytest.mark.usefixtures("company_defaults")
class TestSupplierPriceStatus:
    def test_reports_the_latest_upload_and_change_count_per_supplier(self, api: Client) -> None:
        ...
        body = api.get("/api/purchasing/supplier-price-status/").json()
    ...
    def test_suppliers_without_price_lists_are_omitted(self, api: Client) -> None:
        make_company("No Prices Ltd")
        assert api.get("/api/purchasing/supplier-price-status/").json() == {
            "items": [],
            "total_count": 0,
        }
...
class TestProductMappings:
    ...
    def test_the_list_query_count_does_not_grow_with_the_mapping_count(self, api: Client) -> None:
        ...
        with CaptureQueriesContext(connection) as small:
            api.get("/api/purchasing/product-mappings/")
    ...
    def test_list_puts_unvalidated_mappings_first_with_counts(self, api: Client) -> None:
        ...
        body = api.get("/api/purchasing/product-mappings/").json()
    ...
    def test_blank_nullable_text_is_a_validation_error(self, api: Client, field: str) -> None:
    ...
    def test_an_item_code_absent_from_stock_is_cleared(self, api: Client) -> None:
    ...
    def test_an_unknown_mapping_is_404(self, api: Client) -> None:
        response = api.post(
