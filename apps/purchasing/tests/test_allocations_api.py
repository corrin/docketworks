...
def _receipt(api: Client, po: PurchaseOrder, line_id: str, job_id: str, quantity: str) -> None:
    etag = api.get(f"{PO_URL}{po.id}/").headers["ETag"]
    response = api.post(
...
class TestAllocationListing:
    def test_lists_stock_and_job_allocations_by_line(
        ...
        _receipt(api, po, str(stock_line.id), str(stock_holding_job.id), "5")
        _receipt(api, po, str(job_line.id), str(job.id), "3")

        body = api.get(f"{PO_URL}{po.id}/allocations/").json()
    ...
    def test_a_purchase_order_without_allocations_reports_none(self, api: Client) -> None:
        po = make_purchase_order()
        assert api.get(f"{PO_URL}{po.id}/allocations/").json()["allocations"] == {}
...
class TestTheDetailsReadTakesNoLock:
    ...
    def test_no_statement_locks_a_row(self, api: Client, stock_holding_job: Job) -> None:
        po = make_purchase_order(status="submitted")
        line = make_po_line(po, quantity="5.00", description="Bar", location="Rack 2")
        _receipt(api, po, str(line.id), str(stock_holding_job.id), "5")
        stock = Stock.objects.get(source="purchase_order")

        with CaptureQueriesContext(connection) as queries:
            response = api.get(f"{PO_URL}{po.id}/allocations/stock/{stock.id}/details/")
...
class TestAllocationDetails:
    def test_stock_allocation_details_report_reversal_eligibility(
        ...
        body = api.get(f"{PO_URL}{po.id}/allocations/stock/{stock.id}/details/").json()
    ...
    def test_job_allocation_details_report_the_rates(
        ...
        _receipt(api, po, str(line.id), str(job.id), "2")
        cost_line = CostLine.objects.get(kind="material", cost_set__job=job)

        body = api.get(f"{PO_URL}{po.id}/allocations/job/{cost_line.id}/details/").json()
    ...
    def test_an_unknown_allocation_type_is_400(self, api: Client) -> None:
        po = make_purchase_order()
        response = api.get(f"{PO_URL}{po.id}/allocations/bogus/{uuid4()}/details/")
    ...
    def test_an_allocation_from_another_po_is_404(
        ...
        _receipt(api, other, str(line.id), str(stock_holding_job.id), "1")
        stock = Stock.objects.get(source="purchase_order")
...
class TestAllocationReversal:
    ...
    def test_lost_response_retry_reports_already_reversed_without_writes(
        self, api: Client, stock_holding_job: Job, job: Job, allocation_type: str
    ) -> None:
        po = make_purchase_order(status="submitted")
        line = make_po_line(po, quantity="2.00")
        destination = stock_holding_job if allocation_type == "stock" else job
        _receipt(api, po, str(line.id), str(destination.id), "2")
        ...
        version = api.get(f"{PO_URL}{po.id}/").headers["ETag"]
        first = api.post(
        ...
        listed = api.get(f"{PO_URL}{po.id}/allocations/").json()["allocations"][str(line.id)]
    ...
    def test_reversal_refuses_another_line_without_changing_the_receipt(
        ...
        _receipt(api, po, str(source.id), str(stock_holding_job.id), "2")
        stock = Stock.objects.get(source_purchase_order_line=source)
        response = self._reverse(api, po, str(other.id), "stock", str(stock.id))
        assert response.status_code == 400
        stock.refresh_from_db()
    ...
    def test_reversing_with_a_stale_if_match_is_412_and_writes_nothing(
        ...
        po = make_purchase_order(status="submitted")
        line = make_po_line(po, quantity="5.00", description="Bar")
        stale = api.get(f"{PO_URL}{po.id}/").headers["ETag"]
    ...
    def test_reversing_a_stock_allocation_returns_the_quantity_to_the_line(
        ...
        body = self._reverse(api, po, str(line.id), "stock", str(stock.id)).json()
    ...
    def test_reversing_a_job_allocation_preserves_and_credits_the_cost_line(
        ...
        body = self._reverse(api, po, str(line.id), "job", str(cost_line.id)).json()
    ...
    def test_receipt_reversal_preserves_onward_issues_and_negative_stock(
        ...
        response = self._reverse(api, po, str(line.id), "stock", str(stock.id))
    ...
    def test_reversing_bumps_the_po_etag_even_when_the_status_is_unchanged(
        ...
        _receipt(api, po, str(first.id), str(stock_holding_job.id), "2")
        _receipt(api, po, str(second.id), str(stock_holding_job.id), "2")
        before = api.get(f"{PO_URL}{po.id}/").headers["ETag"]
        stock = Stock.objects.filter(source_purchase_order_line=first).get()

        self._reverse(api, po, str(first.id), "stock", str(stock.id))

        po.refresh_from_db()
        assert po.status == "partially_received"
        assert api.get(f"{PO_URL}{po.id}/").headers["ETag"] != before

    def test_an_unknown_allocation_is_400(self, api: Client) -> None:
        po = make_purchase_order()
        line = make_po_line(po)

        response = self._reverse(api, po, str(line.id), "stock", str(uuid4()))
...
class TestAutomaticAllocationOnFullyReceived:
    ...
    def test_a_patched_line_still_receipts_after_a_price_tbc_toggle(
        ...
        _receipt(api, po, str(line.id), str(stock_holding_job.id), "4")
    ...
    def test_already_allocated_lines_are_not_duplicated(
        ...
        _receipt(api, po, str(line.id), str(stock_holding_job.id), "6")
        etag = api.get(f"{PO_URL}{po.id}/").headers["ETag"]
...
def test_onward_issue_is_not_a_second_receipt_allocation(
    ...
    allocations = api.get(f"{PO_URL}{po.id}/allocations/").json()["allocations"][str(line.id)]
    ...
    response = api.get(f"{PO_URL}{po.id}/allocations/job/{cost.id}/details/")
    assert response.status_code == 404


def test_purchase_order_quantities_and_prices_are_json_numbers(api: Client) -> None:
    ...
    line = api.get(f"{PO_URL}{po.id}/").json()["lines"][0]
