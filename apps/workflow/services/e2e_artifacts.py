...
E2E_SYNC_WINDOWS_FILE = (
    Path(tempfile.gettempdir()) / "docketworks-e2e-sync-windows.json"
)
...
def is_test_company_name(name: str | None) -> bool:
    """True when a company name is reserved for E2E test data."""
    if not name:
        return False
...
def _is_e2e_artifact(
    ...
    """True when this Xero object was created by a finished E2E run."""
...
def drop_e2e_artifacts(
    items: list[InboundXeroObject], entity_type: str
