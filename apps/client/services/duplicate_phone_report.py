"""Data Quality report: phone numbers that break the one-number-one-client rule.

Surfaces the two ways a phone number can be mis-owned, for manual clean-up:
- ``cross_client`` — a number whose effective clients (``COALESCE(client_id,
  contact.client_id)``) number more than one (the grandfathered pre-existing links).
- ``internal_line`` — a client/contact phone method whose number is one of the
...
class DuplicatePhoneReportService:
    ...
    def get_report(self) -> DuplicatePhonesReport:
        ...
        return {
            "duplicate_phones": cross_client + internal_line,
            "summary": {
    ...
    def _cross_client_conflicts(self) -> list[DuplicatePhoneIssue]:
        ...
        conflict_numbers = [
            ...
            .annotate(
                clients=Count(
                    Coalesce("client_id", "contact__client_id"), distinct=True
                )
            )
            .filter(clients__gt=1)
        ]
    ...
    def _owner(method: ClientContactMethod) -> DuplicatePhoneOwner:
        ...
        return {
            ...
            "owner_kind": "client" if method.client_id else "contact",
            "owner_name": method.owner_display_name(),
            "effective_client_id": (
                str(effective_client_id) if effective_client_id else None
            ),
        }
