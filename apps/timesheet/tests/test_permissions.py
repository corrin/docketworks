...
class TimesheetPermissionTests(BaseTestCase):
    ...
    def test_staff_list_blocked_for_normal_user(self) -> None:
        self.client_api.force_authenticate(user=self.normal_user)
        response = self.client_api.get("/api/timesheets/staff/")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_jobs_list_blocked_for_normal_user(self) -> None:
        self.client_api.force_authenticate(user=self.normal_user)
        response = self.client_api.get("/api/timesheets/jobs/")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_payroll_endpoints_blocked_for_normal_user(self) -> None:
        self.client_api.force_authenticate(user=self.normal_user)
        # Post week to Xero
    ...
    def test_stream_payroll_blocked_for_normal_user(self) -> None:
