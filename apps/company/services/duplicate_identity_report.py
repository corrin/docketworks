...
# ── Exact-signal person-name helpers (folded from v1 duplicate_person_report) ──
...
def normalize_person_email(value: str | None) -> str:
    """Normalize an email signal (strip + casefold; "" for missing)."""
...
def _alias_groups() -> dict[str, set[str]]:
    from nicknames import name_triplets  # noqa: PLC0415 -- CSV parse deferred to first use
...
def _build_alias_index(groups: dict[str, set[str]]) -> dict[str, frozenset[str]]:
    ...
    return {name: frozenset(canonical_names) for name, canonical_names in canonical_by_name.items()}
...
def person_names_compatible(
    ...
    *,
    ...
    alias_index = (
        _default_alias_index() if alias_groups is None else _build_alias_index(alias_groups)
    )
...
class DuplicatePersonCompanyLink(TypedDict):
    """One company relationship on a duplicate-person member row."""
...
class DuplicatePersonContactMethod(TypedDict):
    """One contact method on a duplicate-person member row."""
...
class DuplicatePersonSummary(TypedDict):
    """One person, with everything an operator needs to adjudicate a merge."""
...
class DuplicateIdentityEvidence(TypedDict):
    """One corroborating signal shared inside a duplicate group."""
...
class DuplicateCompanyMember(TypedDict):
    """One company inside a duplicate-company group."""
...
class DuplicatePersonMember(DuplicatePersonSummary):
    """One person inside a duplicate-person group (same shape as the summary)."""


class DuplicateCompanyGroup(TypedDict):
    """A group of companies that appear to be one business."""
...
class DuplicatePersonGroup(TypedDict):
    """A group of people that appear to be one human."""
...
class DuplicateIdentityReportSummary(TypedDict):
    """Merge/review group counts by entity kind."""
...
class DuplicateIdentityReport(TypedDict):
    """The full duplicate-identities data-quality report."""
...
# ── Normalization for company signals ────────────────────────────────────
...
def _fingerprint(
    ...
    evidence_values = [
        f"{item['kind']}={item['normalized_value']}:{item['owner_count']}" for item in evidence
    ]
    payload = "|".join(
...
class DuplicateIdentityReportService:
    ...
    def get_report(self) -> DuplicateIdentityReport:
        """Build compact merge/review groups for companies and people."""
        companies = list(Company.objects.filter(merged_into__isnull=True).order_by("id"))
        ...
        methods = list(
            ContactMethod.objects.filter(
                models.Q(company_id__in=company_ids) | models.Q(person_id__in=person_ids)
            )
        )
        ...
        for person in people:
            ...
            for method in methods_by_person[person.id]:
                ...
                if method.method_type == ContactMethod.MethodType.EMAIL:
                    ...
                    raise ValueError(f"Unknown contact method type {method.method_type!r}")
    ...
    def _company_groups(  # noqa: C901, PLR0912 -- Grouping keeps evidence and union traversal together.
        ...
        oversized_members: set[UUID] = set().union(
            *(component for component in auto_components if len(component) > MAX_AUTO_GROUP_SIZE),
            set(),
        )
        accepted_auto = [
            component for component in auto_components if len(component) <= MAX_AUTO_GROUP_SIZE
        ...
        effective_company: dict[UUID, UUID] = {company.id: company.id for company in companies}
    ...
    def _person_groups(  # noqa: PLR0913, PLR0917 -- Grouping inputs expose each identity signal.
        ...
        for pair, matches in pair_evidence.items():
            ...
            second_companies = {
                effective_company[link.company_id] for link in links_by_person[second.id]
            }
    ...
    def _person_rank(
        ...
        from apps.crm.models import PhoneCallRecord  # noqa: PLC0415 -- company <-> crm cycle
        from apps.job.models import Job  # noqa: PLC0415 -- job imports company at module level
