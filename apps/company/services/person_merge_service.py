...
class PersonMergeCounts(TypedDict):
    """Per-model rowcounts moved/collapsed by a person merge."""
...
def _merge_links(source: Person, destination: Person) -> tuple[int, int]:
    destination_links = {
        link.company_id: link for link in CompanyPersonLink.objects.filter(person=destination)
    }
    moved = 0
    collapsed = 0
...
def merge_people(
    ...
    from apps.job.models import Job  # noqa: PLC0415 -- job imports company
    ...
    try:
        with transaction.atomic():
            ...
            methods_moved, methods_collapsed = _merge_contact_methods(source, destination)
            ...
            calls_moved = PhoneCallRecord.objects.filter(person=source).update(person=destination)
            source.delete()
            return {
                "jobs": jobs_moved,
