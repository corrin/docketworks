...
class DuplicatePhoneSummary(TypedDict):
    """Summary counts of duplicate-phone issues."""
...
class DuplicatePhonesReport(TypedDict):
    """The full duplicate-phones data-quality report."""
...
class DuplicatePhoneReportService:
    ...
    def get_report(self) -> DuplicatePhonesReport:
        """Return numbers owned by multiple companies or colliding with internal lines."""
    ...
    def _cross_company_conflicts(self) -> list[DuplicatePhoneIssue]:
        phones = ContactMethod.objects.filter(method_type=ContactMethod.MethodType.PHONE)
        ...
        for method in phones.select_related("company", "person").prefetch_related(
            ...
            owners = companies_by_owner_by_number.setdefault(method.normalized_value, {})
    ...
    def _internal_line_collisions(self) -> list[DuplicatePhoneIssue]:
        ...
        endpoint_labels = dict(
            PhoneEndpoint.objects.filter(is_active=True).values_list("normalized_number", "label")
        )
    ...
    def _owner(method: ContactMethod) -> DuplicatePhoneOwner:
        ...
        return {
            ...
            "effective_company_id": (str(effective_company_id) if effective_company_id else None),
        }
