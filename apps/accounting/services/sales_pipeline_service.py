...
description text parsing — v1 PRs #218 / #220 and migrations 0075 / 0077
...
class SalesPipelineService:
    ...
    def get_report(
        ...
        """Build the full report dict for the period (the wire schema pins shapes)."""
        ...
        snapshot = cls._build_snapshot(events_by_job, jobs_by_id, end_date, warnings_state)
        velocity = cls._build_velocity(
            events_by_job, jobs_by_id, start_date, end_date, warnings_state
        )
        funnel = cls._build_funnel(events_by_job, jobs_by_id, start_date, end_date, warnings_state)
    ...
    def _fetch_events(
        ...
        end_date: date,
        trend_weeks: int,
    ) -> dict[UUID, list[JobEvent]]:
        """Return ``{job_id: [events sorted by timestamp]}`` for the report.

        Covers every job whose events might contribute to any section of the
        report. Narrows the old unbounded fetch in two stages:
        ...
        grouped: dict[UUID, list[JobEvent]] = defaultdict(list)
        total = 0
        for evt in events_qs:
            if evt.job_id is None:
                # Unreachable: the job_id__in filter above excludes orphan rows.
                raise ValueError(f"JobEvent {evt.id} without job_id escaped the job_id__in filter")
    ...
    @classmethod
    def _relevant_job_ids(cls, fetch_start_dt: datetime, end_dt: datetime) -> list[UUID]:
    ...
    @staticmethod
    def _pipeline_stage_job_ids_as_of(end_dt: datetime) -> list[UUID]:
        """Jobs whose latest ``delta_after.status`` at ``end_dt`` is a pipeline stage.

        These are the only jobs whose snapshot replay needs pre-window events
        fetched.
        """
        latest_per_job = (
            JobEvent.objects.filter(
        ...
        return [
            row["job_id"] for row in latest_per_job if row["latest_status"] in PIPELINE_STATUSES
        ]

    @staticmethod
    def _fetch_jobs(job_ids: Iterable[UUID]) -> dict[UUID, Job]:
        ids = list(job_ids)
        if not ids:
            return {}
    ...
    def _working_days_between(start: date, end: date) -> int:
        ...
        d = start
        while d <= end:
            if d.weekday() < 5 and d not in nz_holidays:  # weekday() 5/6 = Sat/Sun
                count += 1
            d += timedelta(days=1)
        return count
    ...
    @classmethod
    def _replay_status_as_of(cls, events: list[JobEvent], cutoff_dt: datetime) -> str | None:
        """Walk events in timestamp order, resolving the status as of ``cutoff_dt``.

        Returns ``None`` when there is no creation anchor with a usable
        ``delta_after.status``.
        """
        anchor_seen = False
    ...
    def _qualifying_approval_event(
        ...
        """Earliest event in ``[period_start_dt, period_end_dt]`` that approves the job.

        A qualifying approval transition is a ``status_changed`` whose
    ...
    @staticmethod
    def _earliest_status_change_to(events: list[JobEvent], target_status: str) -> JobEvent | None:
        for evt in events:
            if evt.event_type != EVT_STATUS_CHANGED:
                continue
            delta = evt.delta_after or {}
    ...
    @staticmethod
    def _earliest_event_of_type(events: list[JobEvent], event_type: str) -> JobEvent | None:
        for evt in events:
    ...
    @staticmethod
    def _summary_hours(summary: dict[str, Any] | None) -> float | None:
        if not summary:
            return None
        h = summary.get("hours")
    ...
    @staticmethod
    def _summary_value(summary: dict[str, Any] | None) -> float | None:
        if not summary:
            return None
        v = summary.get("rev")
    ...
    def _stats(values: list[float]) -> dict[str, Any]:
        ...
        p80_idx = max(0, min(n - 1, round(0.8 * (n - 1))))
        return {
    ...
    def _build_scoreboard(  # noqa: PLR0913, PLR0917 -- Six independent report axes stay explicit.
        ...
        for job_id, events in events_by_job.items():
            ...
            # missing (defensive — should not happen post-v1-PR-#247 backfill),
            ...
            if creation and (qe.timestamp - creation.timestamp) <= INSTANT_APPROVAL_THRESHOLD:
    ...
    def _build_snapshot(
        ...
        for job_id, events in events_by_job.items():
            creation = cls._earliest_event_of_type(events, EVT_JOB_CREATED)
            anchor_status = (creation.delta_after or {}).get("status") if creation else None
            ...
            bucket["jobs"].append(
                {
                    ...
                    "job_number": job.job_number,
                    "name": job.name,
                    "company_name": job.company.name if job.company is not None else "",
                    "hours": hours,
                    "value": value or 0.0,
    ...
    def _build_velocity(
        ...
        for job_id, events in events_by_job.items():
            creation = cls._earliest_event_of_type(events, EVT_JOB_CREATED)
            first_to_awaiting = cls._earliest_status_change_to(events, "awaiting_approval")
            first_approved = cls._earliest_status_change_to(events, "approved")
            first_quote_accepted = cls._earliest_event_of_type(events, EVT_QUOTE_ACCEPTED)
            ...
            approval_candidates = [e for e in (first_approved, first_quote_accepted) if e]
    ...
    def _build_funnel(  # noqa: C901 -- The branch ladder is the bucket taxonomy.
        ...
        for job_id, events in events_by_job.items():
            ...
            had_quote_accepted = any(
                e.event_type == EVT_QUOTE_ACCEPTED and e.timestamp <= cutoff for e in events
            )
            had_rejected = any(
                e.event_type == EVT_JOB_REJECTED and e.timestamp <= cutoff for e in events
            )
    ...
    def _build_trend(
        ...
        for i in range(trend_weeks):
            ...
            week = cls._build_trend_week(events_by_job, jobs_by_id, week_start, week_end)
            weeks.append(week)
