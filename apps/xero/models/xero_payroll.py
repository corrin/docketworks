...
class XeroPayRun(models.Model):
    ...
    pay_run_status = models.CharField(  # noqa: DJ001
        max_length=50, null=True, blank=True
    )  # Draft, Posted, etc.
    pay_run_type = models.CharField(  # noqa: DJ001
        max_length=50, null=True, blank=True
    )  # Scheduled, Unscheduled, etc.

    total_cost = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    total_pay = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    ...
    class Meta:
        ...
        constraints: ClassVar[list[models.BaseConstraint]] = [
            models.CheckConstraint(
                condition=~models.Q(pay_run_status=""), name="pay_run_status_not_blank"
            ),
            models.CheckConstraint(
                condition=~models.Q(pay_run_type=""), name="pay_run_type_not_blank"
            ),
        ]
...
class XeroPaySlip(models.Model):
    ...
    pay_run = models.ForeignKey(XeroPayRun, on_delete=models.CASCADE, related_name="pay_slips")
    ...
    employee_name = models.CharField(  # noqa: DJ001
        max_length=255, null=True, blank=True
    )  # Denormalized for convenience

    # Key payroll amounts
    gross_earnings = models.DecimalField(max_digits=10, decimal_places=2, default=Decimal("0"))
    ...
    class Meta:
        ...
        unique_together: ClassVar[list[list[str]]] = [["pay_run", "xero_employee_id"]]
        constraints: ClassVar[list[models.BaseConstraint]] = [
            models.CheckConstraint(
                condition=~models.Q(employee_name=""), name="employee_name_not_blank"
            ),
        ]
