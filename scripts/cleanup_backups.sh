...
# System python3, no venv: cleanup_backups.py is stdlib-only, and no
# shared venv exists (each release carries its own).
exec python3 "$(dirname "$0")/cleanup_backups.py" "$BACKUP_DIR" "$@"
